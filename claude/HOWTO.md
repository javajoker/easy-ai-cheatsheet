...
The **version-tuning family** under `skills/share/` closes that gap on
demand — and like the evolution loop, it never rewrites a file silently.
...
All three load the same provenance-tagged capability sheets —
`tune-for-opus-4-6`, `tune-for-opus-4-7`, `tune-for-opus-4-8`, and
`tune-for-cc-harness` — and apply their own layer lens on top. When a version
makes a *whole new role* viable, `agent-version-tune` hands off to
`agent-create`, which scaffolds and registers a brand-new agent.
...
instead of a live observation. Full playbook: [SCENARIOS.md Scenario U](SCENARIOS.md#scenario-u--tuning-skillsagentsinstructions-to-a-model-or-harness-updateupgrade).
