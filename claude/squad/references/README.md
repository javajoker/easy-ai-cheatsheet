...
| Squad formations (Elite/Swarm/Critic) | 02 §2 | Routing patterns in `squad-plan` (draft-cheap-fix-expensive; generator+verifier pairs) | **Adapted** — patterns, not preset rosters. |
...
| Deterministic evaluation gates (compiler-as-gate) | 02 §3, 03 §4 | `squad-verify` **gate ladder**: schema → deterministic → in-house judgment | **Adopted** — free gates run first; premium judgment only when they can't decide. |
...
| Auditor Agent (cheap LLM watching handoffs) | 01 §3, 03 §4 | Schema validation is deterministic (free, in `squad-dispatch`); judgment audit stays **in-house** | **Adapted** — a cheap *member* never audits another member (self-grading rule). |
